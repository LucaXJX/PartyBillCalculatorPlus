...
- **消息系統測試工具** 🧪
  ...
  - 控制台測試命令系統
    - `testMessageSystem.testOverdue()` - 測試逾期提醒
    - `testMessageSystem.sendMessage()` - 發送測試消息
    - `testMessageSystem.getMessages()` - 獲取所有消息
    - `testMessageSystem.setPolling(seconds)` - 設置輪詢間隔
  - 完整的測試功能覆蓋
    - 逾期提醒手動觸發
    - 消息發送和接收測試
    - 輪詢機制控制
...
- **測試數據生成系統** 📊
  ...
  - 自動化測試數據生成腳本
    - 21 個測試用戶（符合新密碼要求）
    - 每個用戶平均 10 個相互關聯的測試賬單
    - 多樣化的支付狀態和金額分佈
  - 數據修復工具
    - 修復 `totalAmount` 到 `amount` 字段名稱
    - 修復 `participantsIds` 到 `participantIds` 字段名稱
    - 自動生成收據圖片
...
- **前端顯示問題**
  ...
  - 修復賬單詳情中「個人: undefined」和「共享: undefined」顯示問題
  - 修復 `toFixed()` 錯誤（undefined 值處理）
  - 修復參與者列表顯示問題
  - 統一字段名稱（`participantIds` vs `participantsIds`）

- **TypeScript 編譯問題**

  - 修復 Multer 類型聲明問題
  - 修復 `@types/multer` 導入問題
  - 修復參數類型註解問題
...
- 更新 `TEST_USERS.md` 測試用戶列表
- 創建 `MESSAGE_TESTING_GUIDE.md` 消息系統測試指南
- 更新項目結構文檔
...
- **統一測試用戶配置** 🎯
  ...
  - 所有測試用戶使用統一密碼：`Test123!`
  - 所有測試用戶使用統一郵箱後綴：`@test.com`
  - 創建 `scripts/unify-test-users.js` 自動化腳本
  - 更新 `TEST_USERS.md` 文檔說明統一配置
  - 便於測試和演示，無需記住多個不同密碼
...
- **密碼加密系統** 🛡️
  ...
  - 實現 bcrypt 密碼加密（鹽值輪數：12）
  - 創建 `PasswordUtils` 工具類處理密碼加密和驗證
  - 更新註冊和登入邏輯使用加密密碼
  - 創建 `scripts/migrate-passwords.js` 遷移現有密碼
  - 所有測試用戶密碼已安全加密存儲
  - 解決明文密碼存儲的安全風險
...
- **登入/註冊頁面全新設計** 🎨
  ...
  - 可愛的棕熊互動動畫（手繪 SVG）
    ...
    - 棕熊持續浮動動畫效果
  - 現代化紫色漸變背景（與品牌主題一致）
  - 簡化的 Header（僅保留 Logo 和品牌名稱）
  - 極簡 Footer（僅版權聲明和品牌標語）
  - 白色大圓角卡片設計
  - Google Noto Sans TC 字體（繁體中文優化）

- **即時郵箱格式驗證**

  - 引入 email-validator 開源庫
  - 綠色打勾表示有效郵箱
  - 紅色警告顯示錯誤提示
  - 輸入框邊框顏色即時反饋
...
- 引入依賴：email-validator@2.0.4
- 自定義 SVG 插畫（完全可控）
- CSS 關鍵幀動畫（fadeIn、float）
- JavaScript 互動控制
- 完全響應式設計

### 📝 文檔

- 移除舊的 `public/register-and-login-page.html`
- 保留到 `tests/register-and-login-page.html`（歷史參考）
