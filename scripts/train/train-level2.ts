...
// 訓練配置
const config = {
  dataPath: path.join(__dirname, "../../data/level2-country-classification"),
  batchSize: 32,
  ...
  testSplit: 0.15,
  modelSavePath: path.join(__dirname, "../../models/level2"),
...
async function trainLevel2() {
  ...
  try {
    ...
    // 4. 構建模型
    console.log("\n🏗️  構建模型...");
    const model = buildCountryClassificationModel(
      numClasses,
      config.imageSize
    );
